...
Finally, zoom into the last 4 weeks to spot which teams are pulling ahead and which are falling behind. Toggle between metrics (active users, active days, weekly actions, or per-app usage) to find the pockets of high adoption you can learn from — and the lagging teams that need enablement.
